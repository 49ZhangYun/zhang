...
<template>
  <div class="group-booking">
    <div class="group-booking-top">
      <span class="sp-B">拼团专区</span>
      <span class="sp-M">拼着买更实惠</span>
      <i class="iconfont icon-jiantou1"></i>
    </div>
    <div class="group-booking-bot">
      <swiper ref="mySwiper" :options="swiperOptions">
        <swiper-slide v-for="(item, index) in groupDatas" :key="index">
          <img :src="item.goods_thumb" alt="" class="Imgb" />
          <p>{{ item.goods_name }}</p>
          <div class="price">
              <img
          ...
          />
          <span>￥</span>
          <span class="price-s">{{ item.team_price }}</span>
          </div>
          <p class="old-price">{{ item.shop_price_formated }}</p>
        </swiper-slide>
